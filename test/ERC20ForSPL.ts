...
import type { ERC20ForSPL } from '../typechain-types'

describe('ERC20ForSPL', async () => {
  let owner: any
  let erc20ForSPLAddress = '0x2cc12c6477cA4F4b703c81670C3a85993A6464Ef'
  let erc20ForSPL: ERC20ForSPL
  let solanaProgramAddress: string
  let ownerSolanaPublicKey: string
  ...
  before(async () => {
    ...
    if (ethers.isAddress(erc20ForSPLAddress)) {
      ...
      const contract = await upgrades.deployProxy(ERC20ForSPLFactory, [TOKEN_MINT], {
        kind: 'uups',
        initializer: 'initializeParent',
      })
      erc20ForSPL = contract as unknown as ERC20ForSPL
      await erc20ForSPL.waitForDeployment()
      erc20ForSPLAddress = String(erc20ForSPL.target)
  ...
  it('validate totalSupply is equal', async () => {
  ...
  it('validate decimals are equal', async () => {
    const mintDataOnSolana = await connection.getParsedAccountInfo(new web3.PublicKey(solanaProgramAddress))
    expect(await erc20ForSPL.decimals()).to.eq((mintDataOnSolana.value?.data as any).parsed.info.decimals)
  })
  ...
  //     const tx = await ERC20ForSPL.connect(owner).burn(burnAmount)
  ...
  //   const tx = await erc20ForSPL.connect(user1).transfer(user2.address, transferAmount)
  //   await tx.wait(RECEIPTS_COUNT)
  //
  //   // check from Neon node
  ...
  //
  //     const transferAmount = ethers.parseUnits('5', TOKEN_MINT_DECIMALS)
  //     const tx = await ERC20ForSPL.connect(user2).transfer(user1.address, transferAmount)
  //     await tx.wait(RECEIPTS_COUNT)
  //
  //     // check from Neon node
  ...
  //
  //     const transferAmount = ethers.parseUnits('5', TOKEN_MINT_DECIMALS)
  //     const tx = await ERC20ForSPL.connect(user2).transferSolana(await ERC20ForSPL.solanaAccount(user1.address), transferAmount)
  //     await tx.wait(RECEIPTS_COUNT)
  //
  //     // check from Neon node
  //     const user1BalanceAfter = await ERC20ForSPL.balanceOf(user1.address)
  ...
  //
  //     const tx = await ERC20ForSPL.connect(user2).approve(user1.address, ethers.parseUnits('1', TOKEN_MINT_DECIMALS))
  //     await tx.wait(RECEIPTS_COUNT)
  //
  ...
  //     const solanaUser2TokenBalance = await connection.getTokenAccountBalance(new web3.PublicKey(user2SolanaPublicKey))
  //
  //     const tx = await ERC20ForSPL.connect(user1).transferFrom(user2.address, user1.address, user2Allowance)
  //     await tx.wait(RECEIPTS_COUNT)
  //
  //     // check from Neon node
  //     const user2AllowanceAfter = await ERC20ForSPL.allowance(user2.address, user1.address)
  ...
  //   if (grantedTestersWithBalance) {
  //     const amount = ethers.parseUnits('1', TOKEN_MINT_DECIMALS)
  //     const tx = await ERC20ForSPL.connect(user1).approveSolana(await ERC20ForSPL.solanaAccount(user2.address), amount)
  //     await tx.wait(RECEIPTS_COUNT)
  //     const accountDelegateData = await ERC20ForSPL.getAccountDelegateData(user1.address)
  ...
  //
  //     const amount1 = ethers.parseUnits('2', TOKEN_MINT_DECIMALS)
  //     const tx1 = await ERC20ForSPL.connect(user1).approveSolana(await ERC20ForSPL.solanaAccount(owner.address), amount1)
  //     await tx1.wait(RECEIPTS_COUNT)
  //
  //     const accountDelegateData1 = await ERC20ForSPL.getAccountDelegateData(user1.address)
  ...
  //     const tx2 = await ERC20ForSPL.connect(user1).approveSolana(await ERC20ForSPL.solanaAccount(owner.address), 0)
  //     await tx2.wait(RECEIPTS_COUNT)
  //
  //     const accountDelegateData2 = await ERC20ForSPL.getAccountDelegateData(user1.address)
  ...
  // })
  //
  // it('Malicious change of owner ( supposed to revert )', async () => {
  //   await expect(
  //     ERC20ForSPL.connect(user1).transferOwnership(user1.address),
  //   ).to.be.reverted
  // })
  //
  // it('Malicious contract upgrade ( supposed to revert )', async () => {
  ...
  // })
  //
  // it('Malicious implementation initialize ( supposed to revert )', async () => {
